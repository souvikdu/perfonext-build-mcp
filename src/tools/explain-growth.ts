...
export function registerExplainGrowth(server: McpServer): void {
  server.registerTool(
    ...
    async ({ baselineBuildId, currentBuildId, limit }) => {
      ...
      if (!baseline) {
        throw new Error(
          `Build "${baselineBuildId}" not found. Call explain_growth without IDs to list loaded builds.`,
        );
      }
      ...
      if (!current) {
        throw new Error(
          `Build "${currentBuildId}" not found. Call explain_growth without IDs to list loaded builds.`,
        );
      }
