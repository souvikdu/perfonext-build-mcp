    ...
    'load_webpack_stats',
    {
      ...
      inputSchema: {
        buildId: z
          .string()
          .describe(
            'Build ID returned by load_build_stats; the stats.json is read from that build directory',
          ),
      },
    },
    async ({ buildId }) => {
      ...
      if (!build) {
        throw new Error(
          `Build "${buildId}" not found. Call load_build_stats first and pass the buildId it returns.`,
        );
      }
      ...
      if (!stats) {
        return statsTextResult({
          ...
          nextStep:
            'Call how_to_collect_stats to generate .next/stats.json, then load_webpack_stats again.',
        });
      }
